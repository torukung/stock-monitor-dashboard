"""Rebuild the Watchlist tab from 02_WATCHLIST v7.0 (2026-07-05). Fresh EODHD weekly
closes to Fri Jul 3 embedded; SGX web-approx. v7.0 = Fed-thaw relief rally: ADVANC/
BDMS graduate into positions, KBANK added (new value idea), GULF->Hold, SMIC->Watch,
Cambricon->Trim, CNRE Reduce, Alibaba/BS6 stay Avoid, SCB new Tier-3 watch."""
import json, datetime as dt
ASOF = dt.date(2026, 7, 3)
GEN = "2026-07-05 " + dt.datetime.utcnow().strftime("%H:%M") + " UTC"
...
def interp3(t1,t6):
    ...
    return t1*(t6/t1)**0.4
S = {
 "AMATA.BK":[18.9,19.1,19.7,20.5,20.3,20.6,20.8,21.4,25,24.5,26,26.25,26.5,26.5,26.75],
 "WHA.BK":[4.12,4.22,4.28,4.4,4.38,4.48,4.7,4.68,5,4.9,5.05,4.98,5,5.05,5.2],
 "STECON.BK":[11,12.3,11.9,12,11.5,12,13.9,13.1,14.2,15.7,16.2,17,18.7,18.1,18.4],
 "CK.BK":[14.7,16,16.6,17.2,16.5,17,18.9,17.8,18.2,18,18.9,18.8,20.2,19.7,19.5],
 "GULF.BK":[58.25,59.25,59.5,57.75,56,57.5,60,59.75,60.75,62,67,64,64,59.75,63.5],
 "ADVANC.BK":[367,365,363,353,349,340,350,365,355,353,361,366,357,352,375],
 "BDMS.BK":[18.4,19,18.6,18.7,18.5,18.3,18.4,18.3,18.1,18.2,18.4,18,18.3,18.9,20.1],
 "KBANK.BK":[191,190.5,192,189,190,194,195,196.5,197,201,201,203,207,215,233],
 "SCB.BK":[145.5,144.5,145.5,145,129.5,131,132,133,135,134.5,141,140,141,145,153.5],
 "PTTEP.BK":[159.5,157,151.5,145,149.5,154,148.5,150,151,141.5,144.5,142.5,134,130,131.5],
 "SIRI.BK":[1.36,1.4,1.42,1.41,1.41,1.4,1.41,1.42,1.43,1.43,1.4,1.41,1.44,1.42,1.46],
 "SPALI.BK":[16.3,16.4,16.4,16.5,16.7,16.2,15.3,15.4,15.5,15.5,15.6,15.6,15.7,15.6,16.4],
 "PROUD.BK":[1,0.99,1.01,1.03,1.04,0.98,0.96,0.95,0.92,0.95,0.93,0.94,1.02,1.01,1.03],
 "0700.HK":[493.4,489.2,504.5,510.5,493.4,467.8,471.4,456.4,441.4,427.2,453.2,463.6,440.2,411.8,431.2],
 "3750.HK":[634,627.5,681.5,692,695,608,651,680,686,744.5,711,672.5,708.5,680.5,675.5],
 "0941.HK":[78.05,80.05,81.1,81.45,83.9,84.6,85.45,86.2,85.5,85.15,82.4,81.8,80.1,77.4,76.7],
 "0981.HK":[52.5,51,58.25,59.35,64.3,70.9,73.35,71.15,79.85,81.6,75.65,71.65,76.5,80,77.6],
 "9988.HK":[122.6,118.5,125.5,136.4,131.8,126,139,132.3,127,120.9,122.4,110.2,104.9,89.5,94.1],
 "0883.HK":[29.06,27.02,26.52,26.98,27.9,29.38,26.44,26.42,27.44,26.4,26.54,24.92,22.38,20.86,21.18],
 "3311.HK":[8.49,8.45,8.67,8.69,8.92,9.07,9.38,9.34,8.9,8.75,8.98,8.76,7.59,7.02,7.22],
 "688256.SHG":[1024,1025.7,1210.7,1334,1352.5,1699.96,1182.53,1209,1286,1310,1299.2,1240,1507.46,1458,1353],
 "600111.SHG":[47.27,47.61,51.75,49.22,47.24,53.04,54.75,53.78,51.09,48.62,49.66,49.43,51.4,47.26,48.57],
}
SPARK_FROM, SPARK_TO = "23-Mar-26", "03-Jul-26"
CFG = [
 ...
 dict(tk="WHA",nm="WHA Corporation",ex="SET",sym="WHA.BK",cur="THB",tier=1,th="Industrial land + utilities / data-center",
   ...
   note="⚠️ TAKE PROFIT — above 1-mo target 5.10 (+3% 7-day). DC + utilities. Trim, trail 4.50."),
 dict(tk="STECON",nm="Stecon Group",ex="SET",sym="STECON.BK",cur="THB",tier=1,th="Construction / new data-center unit",
   ...
   note="Through 6-mo zone; 7-day +1.7%. DC unit + PPP/Land-Bridge bids. Keep trim, stop 16."),
 dict(tk="CK",nm="CH. Karnchang",ex="SET",sym="CK.BK",cur="THB",tier=1,th="Construction / PPP + Land Bridge",
   ...
   note="Uptrend intact but 1-mo momentum cooled (−0.5%). Hold, stop 17."),
 dict(tk="GULF",nm="Gulf Development",ex="SET",sym="GULF.BK",cur="THB",tier=1,th="Power / AI-DC + renewables",
   ...
   note="↗️ RE-CONFIRMING — bounced +6.3% wk off entry. THB140bn AI/DC + renewables; lower oil = lower gas cost. Hold, stop 56."),
 dict(tk="ADVANC",nm="Advanced Info Service",ex="SET",sym="ADVANC.BK",cur="THB",tier=1,th="Thai telecom / defensive dividend",
   ...
   note="↗️ GRADUATED — Top Idea #1. Defensive telecom + dividend, broke out +6.5% wk on inflows. ~4.6% yield, FY26 EPS +10.7%, consensus TP ~392."),
 dict(tk="BDMS",nm="Bangkok Dusit Medical",ex="SET",sym="BDMS.BK",cur="THB",tier=1,th="Thai healthcare / defensive",
   ...
   note="↗️ GRADUATED — Top Idea #3. Healthcare defensive, AI-immune; +10.4% 1-mo. Strong-Buy, TP ~26 (~+29%)."),
 dict(tk="KBANK",nm="Kasikornbank",ex="SET",sym="KBANK.BK",cur="THB",tier=1,th="Thai bank / value + dividend",
   conv=4,st="Hold",entry=233,stop=214,t=[245,265,280],cagr=0.10,ed="2026-07-05",cs="2026-07-05",
   note="↗️ NEW — Top Idea #2. Bank leadership on BoT GDP upgrade + loan growth + inflows; +14.2% 1-mo. ~P/E 8, ~4% yield, payout ~40%."),
 dict(tk="S63",nm="ST Engineering",ex="SGX",sym="S63.SI",cur="SGD",tier=1,th="Defense / engineering",
   ...
   note="Below entry (ex-div). Record ~S$33bn order book; defense backlog intact. Hold. SGX price approx."),
 dict(tk="0981",nm="SMIC",ex="HKEX",sym="0981.HK",cur="HKD",tier=2,th="China foundry / semi self-sufficiency",
   conv=3,st="Watch",entry=79.45,stop=66,t=[82,90,100],cagr=0.12,ed="2026-05-15",cs="2026-07-05",
   note="🔍 WATCH — relative strength FADED (1-mo −6.4%, 7-day −3.0%); round-tripped the Jun 30 spike. Hold core, no adds. Cut ⭐⭐⭐⭐→⭐⭐⭐."),
 dict(tk="688256",nm="Cambricon Technologies",ex="SSE",sym="688256.SHG",cur="CNY",tier=2,th="China AI chips",
   ...
   note="🔍 Round-trip — spiked ¥1,595 then back to ¥1,353 (7-day −7.2%). ≤5-7% max, P/E extreme. Hold core, stop ¥1,250."),
 dict(tk="600111",nm="China Rare Earth (CNRE)",ex="SSE",sym="600111.SHG",cur="CNY",tier=2,th="Rare earth / Nov Wave-2 catalyst",
   ...
   note="⚠️ REDUCE — relief-bounced above ¥44 (+2.8% wk) but still below entry, 1-mo −2.5%. Exit on a close <¥44."),
 dict(tk="9988",nm="Alibaba Group",ex="HKEX",sym="9988.HK",cur="HKD",tier=2,th="China tech / AI (Qwen) / cloud",
   ...
   note="🚨 REMOVED — bounced +5.1% wk but still below the HK$100 stop; 1-mo −25.7%. Do not re-add on the bounce."),
 dict(tk="U96",nm="Sembcorp Industries",ex="SGX",sym="U96.SI",cur="SGD",tier=3,th="Power-for-AI / renewables",
   conv=3,st="Watch",entry=None,stop=None,t=None,cagr=0,ed=None,cs="2026-06-15",approx=6.20,
   note="WATCH — power-for-AI + renewables without semi risk; eased back this week. ~P/E 9, ~4% yield, TP ~6.89. Accumulate ~S$6.10-6.30. SGX approx."),
 dict(tk="0700",nm="Tencent Holdings",ex="HKEX",sym="0700.HK",cur="HKD",tier=3,th="China tech / AI / cloud",
   ...
   note="DE-ELEVATED — ~HK$431, bounced +4.7% wk but 1-mo −7.5% in a China-tech bear market. Cheap (P/E ~16) but a low-quality bounce; re-engage only on momentum confirmation."),
 dict(tk="3750",nm="CATL (HK)",ex="HKEX",sym="3750.HK",cur="HKD",tier=3,th="EV battery / storage",
   conv=3,st="Watch",entry=None,stop=None,t=None,cagr=0,ed=None,cs="2026-06-27",
   note="DE-ELEVATED — 1-mo −13.2%. Storage super-cycle intact long-term; near-term momentum broken. Re-engage on a momentum reset."),
 dict(tk="0941",nm="China Mobile",ex="HKEX",sym="0941.HK",cur="HKD",tier=3,th="China AI-compute / dividend",
   ...
   note="WATCH — 1-mo −10.3%. ~5% dividend, ~10x P/E. 'Defensive' not working; accumulate only if it stops falling."),
 dict(tk="AJBU",nm="Keppel DC REIT",ex="SGX",sym="AJBU.SI",cur="SGD",tier=3,th="Data-center REIT",
   ...
   note="NOT triggered — wants yield >5% or price <S$2.10 (now ~2.28). A Fed thaw would help rate-sensitives — watch. SGX approx."),
 dict(tk="PTTEP",nm="PTT Exploration & Prod.",ex="SET",sym="PTTEP.BK",cur="THB",tier=3,th="Oil & gas / E&P",
   ...
   note="DE-ELEVATED — ~THB 131.5, 1-mo −8.0%. Oil leverage is a headwind. Re-engage only on a Hormuz/supply shock."),
 dict(tk="0883",nm="CNOOC",ex="HKEX",sym="0883.HK",cur="HKD",tier=3,th="Oil major",
   conv=2,st="Watch",entry=None,stop=None,t=None,cagr=0,ed=None,cs="2026-06-08",
   note="DE-ELEVATED — ~HK$21.18, 1-mo −22.3%. Re-engage only if oil re-rates up."),
 dict(tk="3311",nm="China State Constr. Intl",ex="HKEX",sym="3311.HK",cur="HKD",tier=3,th="Construction / BRI",
   conv=2,st="Watch",entry=None,stop=None,t=None,cagr=0,ed=None,cs="2026-06-08",
   note="~HK$7.22, 1-mo −19.8%; needs a fresh Belt-and-Road mega-project win. Watch."),
 dict(tk="SCB",nm="SCB X",ex="SET",sym="SCB.BK",cur="THB",tier=3,th="Thai bank / high yield",
   ...
   note="NEW WATCH — value/yield backstop if KBANK extends. ~THB 153.5, +11.6% 1-mo; ~8% yield but payout high (~80%) → KBANK preferred."),
 dict(tk="SIRI",nm="Sansiri",ex="SET",sym="SIRI.BK",cur="THB",tier=3,th="Property",
   conv=2,st="Watch",entry=None,stop=None,t=None,cagr=0,ed=None,cs="2026-06-08",
   note="Small bounce (+3.5% 1-mo); Thai property still weak. Not yet."),
 ...
 dict(tk="SPALI",nm="Supalai",ex="SET",sym="SPALI.BK",cur="THB",tier=3,th="Property",
   ...
   note="AVOID (yield trap) — 35% EPS decline in 2025; +5.8% 1-mo bounce but still weak. Not a buy."),
...
data={"meta":{"title":"STOCK Monitor — Daily","generated":GEN,"asof":"2026-07-03","version":"v7.0","awaiting_sync":awaiting,
              "count":len(holds),"note":"Prices: EODHD main-board closes to Fri Jul 3 (TH/HK/China-A). SGX web-approx (labelled). NOT financial advice."},
      "holdings":holds}
...
wl={"meta":{"title":"STOCK Monitor — Daily","owner":"ToR","schema_version":2,"source_version":"v7.0 (2026-07-05)",
...
print("v7.0 sync done",GEN,"| holdings",len(holds),"| awaiting",awaiting)
